...
- **Market-data replay:** a session starts with a self-describing `SESSION_START` record (instrument rules and book sizing). The live session journals every raw stream message before decoding it, every snapshot it applies (with its fetch time), and every connection reset. Decoding and book maintenance live in one pure component (`MdBooks`) that both the live session and replay drive, so a replay reproduces the same books and status changes without any other configuration. `bowst-md --journal DIR` records and `bowst-md --replay DIR` replays.

## Consequences
- Journaling a typical 459-byte message costs about 20 ns on the hot path (the byte-ring hand-off).
- Tested: a 2,000-record round trip, rotation across files, a torn tail, truncation in the middle, every single-bit flip in a record, a missing segment, drop accounting with the gap marker, no overwriting, zero allocations on append, a segment written by an independent implementation of the documented format, and a mock-venue session (with a dropped frame, a disconnect and a resync) whose replay produces exactly the live event sequence. The record decoder is fuzzed in CI.
